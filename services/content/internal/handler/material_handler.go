...
// GeneratePresignedURL handles POST /api/v1/materials/:id/presign
func (h *MaterialHandler) GeneratePresignedURL(c echo.Context) error {
	userID, err := getUserID(c)
	if err != nil {
		return echo.NewHTTPError(http.StatusUnauthorized, "unauthorized")
	}
	...
	var req service.PresignRequest
	if err := c.Bind(&req); err != nil {
		logger.Error("failed to bind presign request", zap.Error(err))
		return echo.NewHTTPError(http.StatusBadRequest, "invalid request body")
	}

	// Default action to "upload" if not specified
	if req.Action == "" {
		req.Action = "upload"
	}
	...
	if req.Action != "upload" && req.Action != "download" {
		return echo.NewHTTPError(http.StatusBadRequest, "invalid action, must be 'upload' or 'download'")
	}
	...
	presignResp, err := h.service.GeneratePresignedURL(c.Request().Context(), userID, materialID, &req)
	if err != nil {
		if err == service.ErrMaterialNotFound {
			return echo.NewHTTPError(http.StatusNotFound, "material not found")
		}
