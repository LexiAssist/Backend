# LexiAssist — AI Monolith Integration Plan
...
|---|----------|------|-------|--------|
| 1 | P0 | Fix requirements.txt encoding (UTF-16 → UTF-8) | `lexiassist-ai-main/requirements.txt` | ✅ DONE |
| 2 | P0 | Create Dockerfile for AI monolith | `lexiassist-ai-main/Dockerfile` | ✅ DONE |
...
| 5 | P1 | Update database.py to use `ai` schema | `lexiassist-ai-main/database.py` | ✅ DONE |
| 6 | P1 | Add `AI_SERVICE_URL` to Gateway env in compose | `infra/docker-compose.yml` | ✅ DONE |
| 7 | P2 | Make Weaviate connection lazy in reading_engine.py | `lexiassist-ai-main/reading_assistant/reading_engine.py` | ✅ DONE |
| 8 | P2 | Fix `_chunk_text` call bug in reading_engine.py | `lexiassist-ai-main/reading_assistant/reading_engine.py` | ✅ DONE |
| 9 | P2 | Make Weaviate connection lazy in lexicore.py | `lexiassist-ai-main/lexicore.py` | ✅ DONE |
...
| 12 | P0 | Add /health endpoint to AI monolith | `lexiassist-ai-main/api.py` | ✅ DONE |
...
### Task 9 — Make Weaviate connection lazy in lexicore.py

**Problem:** `lexicore.py` has global `wclient`, `collection`, `model` variables that are only initialized when `_init_clients()` is called from `run_cli()`. The web server path (`api.py`) imports `LexiEngine` from `lexicore.py` — the globals remain `None`. If any web endpoint tries to use `contextual_chat` or `ingest_material`, it will crash with `NoneType` errors.
