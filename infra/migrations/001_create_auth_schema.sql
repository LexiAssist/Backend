...
CREATE INDEX IF NOT EXISTS idx_jwt_keys_active ON auth.jwt_keys(is_active) WHERE is_active = TRUE;
...
CREATE TABLE IF NOT EXISTS auth.user_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    refresh_token_id UUID REFERENCES auth.refresh_tokens(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_user_sessions_revoked_at ON auth.user_sessions(revoked_at);
...
BEGIN
    ...
    DELETE FROM auth.password_resets WHERE expires_at < CURRENT_TIMESTAMP AND used = FALSE;
    DELETE FROM auth.refresh_tokens WHERE expires_at < CURRENT_TIMESTAMP AND revoked = TRUE;
END;
$$ LANGUAGE plpgsql;
